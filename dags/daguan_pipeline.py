...
_daguan_root = os.path.join(os.environ['HOME'], 'airflow')
_data_root = os.path.join(_daguan_root, 'data/nlp_data')
# Python module file to inject customized logic into the TFX components. The
# Transform and Trainer both require user-defined functions to run successfully.
_daguan_module_file = os.path.join(_daguan_root, 'dags/taxi_utils.py')
# Path which can be listened to by the model server.  Pusher will output the
# trained model here.
_serving_model_dir = os.path.join(_daguan_root, 'saved_models/taxi')

# Directory and data locations.  This example assumes all of the chicago taxi
# example code and metadata library is relative to $HOME, but you can store
# these files anywhere on your local filesystem.
_tfx_root = os.path.join(_daguan_root, 'tfx')
...
def _create_pipeline():
  ...
  transform = Transform( # Step 4
      ...
      module_file=_daguan_module_file) # Step 4
